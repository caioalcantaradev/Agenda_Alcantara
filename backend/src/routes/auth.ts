...
authRouter.post("/login", async (req, res) => {
  ...
  return res.json({ token, user: { id: user.id, email: user.email, name: user.name, mustChangePassword: user.mustChangePassword } });
});

authRouter.get("/me", requireAuth, async (req, res) => {
  const user = await User.findById(req.user!.id).lean();
  if (!user) return res.status(401).json({ message: "Não autenticado" });
  return res.json({ user: { id: String(user._id), email: user.email, name: user.name, mustChangePassword: user.mustChangePassword } });
});
...
authRouter.post("/change-password", requireAuth, async (req, res) => {
  ...
  if (user.mustChangePassword) {
    ...
    const ok = await (await import("bcryptjs")).default.compare(currentPassword, user.passwordHash);
